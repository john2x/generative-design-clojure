...
(ns generative-design-clojure.principles.P_1_2_3_04.P_1_2_3_04
  (:use quil.core
        quil.helpers.seqs
...
(defn draw []
  ...
  (let [counter (atom 0), ; count tiles
    ...
    (doseq [i (reverse (range row-count))
      ...
      (doseq [ii (range (count @parts))
        ...
        (when (< (random 1.0) 0.45)
          (let [x (+
                    (map-range @sum-parts-now 0 @sum-parts-total 0 (width))
                    (random -10 10)),
                y (+ (* row-height i) (random -10 10)),
                w (+
                    (* (map-range (@parts ii) 0 @sum-parts-total 0 (width)) -1)
                    (random -10 10)),
                h (* row-height 1.5)]
            (begin-shape)
            (fill 0 0 0 180)
